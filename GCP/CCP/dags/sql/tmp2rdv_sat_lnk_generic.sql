...
JOIN {project}.{tgt_dataset_nm}.{tgt_datastore_nm} E ON C.{src_datastore_hashkey_nm} = E.{tgt_datastore_hashkey_nm} 
WHERE (E.an_current_flag = 1 AND C.ah_checksum <> E.ah_checksum )) S ON T.{tgt_datastore_hashkey_nm} = S.hash_key AND T.an_current_flag = 1 
WHEN MATCHED AND S.ah_checksum <> T.ah_checksum THEN UPDATE SET T.an_current_flag =0 
WHEN NOT MATCHED BY SOURCE AND T.an_current_flag=1 THEN UPDATE SET T.an_current_flag =0 
